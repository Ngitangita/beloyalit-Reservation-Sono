...
const schema = yup.object({
  ...
  confirm: yup
    .string()
    .oneOf([yup.ref("mot_de_passe")], "Les mots de passe doivent correspondre")
    .required("La confirmation est obligatoire"),
});

export const SignUp = () => {
  const { signup } = useApiAuth<{ user: import("~/types/types").UserType }>();
      ...
      execute(
        ...
        }),
        {
          onSuccess: res => {
            ...
            navigate(role === 1 ? "/admin" : "/client", { replace: true });
          },
      ...
      <div className="overflow-hidden w-[1250px] flex flex-col justify-start items-start z-50 pl-10 pr-10">
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="h-auto p-6 flex flex-row flex-wrap gap-4 justify-between items-center">
              ...
              <div key={field} className="flex flex-col gap-1">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder={label}
                    disabled={pending}
                    className={`w-[400px] sm:w-[550px] p-2 pr-10 border rounded outline-[#18769C] ${error ? "border-red-500" : "border-[#18769C]/50"}`}
                  />
                  {(isPwd || isConf) && (
                    <button type="button" onClick={() => isPwd ? setShowPassword(p => !p) : setShowConfirm(p => !p)} 
                    className="absolute inset-y-0 right-2 flex items-center text-gray-500" tabIndex={-1}>
          ...
          <div className="flex flex-col items-center w-[1100px]">
            <button type="submit" disabled={isSubmitting || pending} className="w-[400px] sm:w-[400px] p-2 rounded hover:bg-gradient-to-l hover:from-[#18769C] hover:to-#18769C]/20 mt-4 cursor-pointer bg-gradient-to-r from-[#18769C] to-[#18769C]/20 text-xl text-white">
