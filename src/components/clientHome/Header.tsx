    ...
    <header className="fixed top-0 w-full bg-[#1E2939] text-white z-50">
      <div className="flex justify-between items-center p-4 pr-10">
        <div className="flex flex-row items-center gap-2">
          <img src="/logo-blit.png" alt="Logo BlitSono" className="w-24 h-24 object-cover rounded-full" />
          <div className="text-center font-bold text-3xl leading-none flex flex-col italic font-serif">
            <span>beloya<span className="text-[#18769C]">lit </span></span>
        ...
        <div className="hidden sm:flex items-center gap-20">
            ...
            {menuItems.map(item =>
              (item.title !== "Espace client" || isAuth) && (
                <Link key={item.path} to={item.path}>
                  <span className={`relative inline-block group rounded-full transition duration-300 ease-in-out p-4 ${isActive(item.path) ? "font-semibold underline underline-offset-[5px]" : "bg-base-200 text-gray-400 hover:text-white"}`}>
                    {item.title}
                    {!isActive(item.path) && (
          ...
          <div className="flex gap-4">
            ...
            <Link to="/sign-in"><FaUser className="text-2xl" /></Link>
            {isAuth && <button onClick={logout}><MdLogin className="text-2xl" /></button>}
          </div>
        </div>
      </div>
      {open && (
        <div className="sm:hidden bg-[#1E2939] px-4 pb-4">
            ...
            {menuItems.map(item =>
              (item.title !== "Espace client" || isAuth) && (
                <Link key={item.path} to={item.path} onClick={() => setOpen(false)}>
                  <span className={`block px-4 py-2 rounded-full ${isActive(item.path) ? "font-semibold bg-gray-700" : "text-gray-300 hover:bg-gray-700 hover:text-white"}`}>
                    {item.title}
                  </span>
                </Link>
              )
          ...
          <div className="flex gap-4 mt-4">
            <Link to="/basket" onClick={() => setOpen(false)}><FaShoppingCart className="text-2xl" /></Link>
            <Link to="/sign-in" onClick={() => setOpen(false)}><FaUser className="text-2xl" /></Link>
            {isAuth && <button onClick={logout}><MdLogin className="text-2xl" /></button>}
          </div>
        </div>
      )}
